...
export default function Candidate() {
    ...
    const getData = async () => {
        try {
            const response = await axios.get(process.env.REACT_APP_API + `api_House_of_Representatives.php/?xCase=2`
            );
    ...
    const deleteData = (id) => {
        axios
            .post(process.env.REACT_APP_API + `api_House_of_Representatives.php/${id}/?xCase=3`)
    ...
    const handleSearchChange = (event) => {
        ...
        if (Datas && Datas.length > 0) {
            const filteredResults = Datas.filter((Data) =>
                (Data.prefix || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                (Data.Representatives_firstname || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                (Data.Representatives_lastname || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                (Data.Political_Party || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const columns = [
        ...
        { name: 'สังกัด', selector: 'Political_Party', sortable: true },
        ...
        { name: 'เขตพื้นที่', selector: 'constituency', sortable: true },
        {
            ...
            cell: (row) => (
                <div className='d-flex align-items-center'>
                    <Link to={`${row.Representatives_id}/Personal_House_of_Representatives`}>
                        <button className='btn btn-link'>
                            <i className='fas fa-address-card fs-5'></i>
                        </button>
                    </Link>
                    {/* <Link to={`${row.Representatives_id}/Edit_House_of_Representatives`}>
                        <button className='btn btn-link'>
                            <i className='far fa-edit wa text-warning fs-5'></i>
                        </button>
                    </Link> */}
                    <button className='btn btn-link' onClick={() => showAlertDelete(row.Representatives_id)}>
        ...
        <div>
            <div className="row">
                ...
                <div className="col-10">
                    <Navbar />
                    <strong><h1 className='text1 text-center'>สมาชิกสภาผู้แทนราษฎร( สส. )</h1></strong>
                    <div className="card-type col-11 mx-lg mx-auto">
                        <Link to="/Insert_House_of_Representatives"><button className='btn btn-info mb-lg-3'>เพิ่มข้อมูล</button></Link>
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">สมาชิกสภาผู้แทนราษฎร( สส. )</h3>
                            </div>
                            <div class="card-body">
