...
export default function Candidate_Provincial_Council() {
    ...
    const deleteData = (id) => {
        axios
            .post(process.env.REACT_APP_API + `api_Provincial_Council.php/${id}/?xCase=3`)
    ...
    const handleSearchChange = (event) => {
        ...
        const filteredResults = Datas.filter(Data =>
            Data.provincial_firstname.toLowerCase().includes(searchTerm.toLowerCase()) ||
            Data.provincial_lastname.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        { name: 'เขตพื้นที่', selector: 'constituency', sortable: true },
        {
            ...
            cell: (row) => (
                <div className='d-flex align-items-center'>
                    <Link to={`${row.provincial_id}/Personal_Provincial_Council`}>
                        <button className='btn btn-link'>
                            <i className='fas fa-address-card fs-5'></i>
                        </button>
                    </Link>
                    {/* <Link to={`${row.provincial_id}/Edit_Provincial_Council`}>
                        <button className='btn btn-link'>
                            <i className='far fa-edit wa text-warning fs-5'></i>
                        </button>
                    </Link> */}
                    <button className='btn btn-link' onClick={() => showAlertDelete(row.provincial_id)}>
        ...
        <div>
            <div className="row">
                ...
                <div className="col-10">
                    <Navbar />
                    <strong><h1 className='text1 text-center'>สมาชิกสภาจังหวัด( สจ. )</h1></strong>
                    <div className="card-type col-10 mx-lg mx-auto">
                        <Link to="/Insrt_Provincial_Council"><button className='btn btn-info mb-lg-3'>เพิ่มข้อมูล</button></Link>
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">สมาชิกสภาจังหวัด( สจ. )</h3>
                            </div>
                            <div class="card-body">
